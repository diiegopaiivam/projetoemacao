<?php 
  ...
  if($estrategia > 0)
    header("Location: altestrategias.php?idsum=$idsum&idestrategia=$estrategia");

  $estrategiadao = new EstrategiaDAO();
  $listagemobjetivo = $estrategiadao->DadosObjetivo($idsum);

  $i = 0;

?>
      <div class="panel-header">
        <h1 class="text-center text-white">Cadastrar estratégias</h1>
      </div>
      <div class="content">
        <div class="row">
          <div class="col">
            <div class="card">
              ...
              <?php 
                ...
                if(isset($idobjetivo)){
                  ...
                  foreach ($idobjetivo as $objetivo) {
                    ...
                    $valor = str_replace(['.', ','], ['', '.'], $valor[$i]);

                    $estrategialista = [1 => $idsum, 2 => $objetivo, 3 => utf8_decode($local[$i]), 
                                        4 => $cdata->DataEUA($data[$i]), 5 => utf8_decode($responsavel[$i]), 6 => $valor ?? 0, 7 => utf8_decode($status[$i])];

                    $result = $estrategiadao->CadastrarEstagio($estrategialista);
              ...
              <div class="card-header">
                ...
                <h5 class="title">Cadastrar um nova estratégia :)</h5>
              </div>
              <div class="card-body">
                <form method="POST">
                  ...
                  <div class="row mt-5">
                    <div class="col-md-4 pr-1">
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" disabled value="<?php echo $row->objetivo ?>" >
